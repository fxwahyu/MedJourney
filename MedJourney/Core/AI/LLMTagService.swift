...
/// Result of an AI tagging pass: short observational tags, plus a structured
/// markdown analysis for checkup entries.
struct AITagResult {
    let tags: [String]
    let analysis: String?
}

/// Generates AI-powered health tags (and checkup analysis) from a journal entry.
protocol AITagServiceProtocol {
    func generateTags(for entry: JournalEntry, ocrText: String?) async throws -> AITagResult
}

/// Builds tagging / checkup-analysis prompts and parses the LLM response.
/// All network calls route through `LLMGateway`.
final class LLMTagService: AITagServiceProtocol {
    ...
    func generateTags(for entry: JournalEntry, ocrText: String? = nil) async throws -> AITagResult {
        ...
        guard !prompt.isEmpty else { return AITagResult(tags: [], analysis: nil) }

        let isCheckup = entry.entryType == .checkup
        let raw = try await LLMGateway.shared.complete(prompt: prompt, jsonMode: isCheckup)

        return isCheckup
            ? parseCheckupJSON(from: raw)
            : AITagResult(tags: parseTags(from: raw), analysis: nil)
    }
    ...
    private func buildPrompt(for entry: JournalEntry, ocrText: String?) -> String {
        ...
        case .journal:    return journalPrompt(for: entry)
        case .checkup:    return checkupPrompt(for: entry, ocrText: ocrText)
        case .medication: return ""
        }
    }

    private func journalPrompt(for entry: JournalEntry) -> String {
        var lines: [String] = []
        lines.append("""
        You are a health data assistant. Extract ONLY the symptoms the user actually experienced from this journal entry.
        Return ONLY a comma-separated list of 1-5 short symptom tags. No explanation.

        TAG FORMAT — follow exactly:
        - Each tag is a plain, human-readable symptom in everyday English (e.g. 'headache', 'fatigue', 'dizziness', 'nausea', 'blurry vision', 'insomnia', 'stress', 'anxiety').
        - Use spaces, NOT hyphens (write 'blurry vision', never 'blurry-vision').
        - Lowercase, 1-3 words each.

        WHAT COUNTS AS A SYMPTOM (include these):
        - Physical symptoms: headache, nausea, dizziness, fatigue, blurry vision, sore throat, stomach cramp, back pain, lightheadedness, etc.
        - Mental/sleep states: stress, anxiety, insomnia, poor sleep, low energy.
        - Abnormal vitals only (skip normal values): high blood pressure (systolic >140 or diastolic >90), elevated heart rate (>100 bpm), fever (>37.5°C), low temperature (<36°C).

        DO NOT INCLUDE (these are NOT symptoms):
        - Activities or habits: exercise, walking, hydration, diet.
        - Status/meta words: monitoring, routine, improving, concern, progress, positive, management, adherence, reminder.
        - Medication names or events, appointments, or anything that is not a felt symptom.
        - Diagnoses or disease names (e.g. write 'high blood pressure', never 'hypertension'; 'headache', never 'migraine').

        If the entry describes a good day with no symptoms, return an empty list.

        Entry data:
        Mood: \(entry.title)
        """)
        if !entry.content.isEmpty            { lines.append("Description: \(entry.content)") }
    ...
    private func checkupPrompt(for entry: JournalEntry, ocrText: String?) -> String {
        var lines: [String] = []
        lines.append("""
    ...
    /// Parses the checkup JSON response into tags + a markdown analysis string
    /// (`### heading` + `• item` lines, the format `MarkdownAnalysisView` renders).
    private func parseCheckupJSON(from raw: String) -> AITagResult {
        ...
        if let sections = json["sections"] as? [[String: Any]] {
            for section in sections {
                guard let heading = section["heading"] as? String,
                      let items = section["items"] as? [String] else { continue }
        ...
        let analysis = markdownParts
            .joined(separator: "\n")
            .trimmingCharacters(in: .whitespacesAndNewlines)
        return AITagResult(tags: tags, analysis: analysis.isEmpty ? nil : analysis)
    }
