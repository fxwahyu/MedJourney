//
//  OpenAITagService.swift
//  MedJourney
//
//  Core/AI — Generates health tags and deep analysis using OpenAI's GPT-4o API
//

import Foundation

/// Calls OpenAI's Chat Completions API (GPT-4o) to extract health tags and structured
/// analysis from a journal entry. Mirrors GeminiTagService for direct comparison.
final class OpenAITagService: AITagServiceProtocol {
    ...
    func generateTags(for entry: JournalEntry, ocrText: String? = nil) async throws -> AITagResult {
        ...
        let raw = try await callOpenAI(prompt: prompt, isCheckup: entry.entryType == .checkup)
    ...
    private func buildPrompt(for entry: JournalEntry, ocrText: String?) -> String {
        ...
        case .journal:
            ...
            lines.append("- If vitals are outside normal range, describe the observation — never infer a diagnosis.")
            lines.append("")
            lines.append("Rules:")
    ...
    private func callOpenAI(prompt: String, isCheckup: Bool) async throws -> String {
        guard !prompt.isEmpty else { return "" }
        print("🤖 [OpenAITagService] Starting \(model) generation. Prompt length: \(prompt.count)")
        ...
        print("🤖 [OpenAITagService] Sending request to OpenAI API...")

        let (data, response) = try await session.data(for: request)

        let statusCode = (response as? HTTPURLResponse)?.statusCode ?? 0
        print("🤖 [OpenAITagService] Received response. Status code: \(statusCode)")

        guard let http = response as? HTTPURLResponse, http.statusCode == 200 else {
            if let errorString = String(data: data, encoding: .utf8) {
                print("🤖 [OpenAITagService] ❌ API Error: \(errorString)")
            }
        ...
        guard
            ...
            print("🤖 [OpenAITagService] ❌ Failed to parse expected JSON structure.")
            throw AITagError.parseError
        }

        print("🤖 [OpenAITagService] ✅ Successfully extracted text. Length: \(text.count)")
        return text
    }
    ...
    private func parseJSON(from raw: String) -> AITagResult {
        ...
        print("🤖 [OpenAITagService] Parsed \(tags.count) tags and \(markdownParts.count) lines of analysis.")
